...
//		0表示没有付款、1表示已付款即将发货 2表示确认收货 3表示交易成功
		...
		for (Entry<Integer, CartItem> entry : cartItems.entrySet()) {
			...
			orderitem.getProduct().setStock(orderitem.getProduct().getStock() - orderitem.getCount());
			productService.updateStock(orderitem.getPid(),orderitem.getProduct().getStock());
		}
