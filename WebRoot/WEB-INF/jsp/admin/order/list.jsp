									...
									<c:forEach items="${pageBean.list}" var="o" varStatus="status">
										<tr onmouseover="this.style.backgroundColor = 'white'"
											...
											<td style="CURSOR: hand; HEIGHT: 22px" align="center"
												...
												<c:if test="${o.state==1}">
													已购买，<a href="${ pageContext.request.contextPath }/admin/adminOrder_updateState.action?oid=${o.oid}&status=2"><font color="blue">进行配送</font></a>
												</c:if>
												<c:if test="${o.state==2}">
													已发货等待付款
												</c:if>
												<c:if test="${o.state==3}">
													已确认收货，<a href="${ pageContext.request.contextPath }/admin/adminOrder_updateState.action?oid=${o.oid}&status=4"><font color="blue">收到货款</font></a>
												</c:if>
												<c:if test="${o.state==4}">
													订单完成
												</c:if>
